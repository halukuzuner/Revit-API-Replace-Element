...
{
	Result Command::Execute(ExternalCommandData^ commandData, String^% message, Autodesk::Revit::DB::ElementSet^ elements)
	{	
		//	To handle main Revit window.
		if (nullptr == Command::_hWndRevit)
		{
			Process^ process = Process::GetCurrentProcess();

			IntPtr h = process->MainWindowHandle;
			Command::_hWndRevit = gcnew WindowHandle(h);
		}
		...
		try
		{
			...
			if (mainForm == nullptr || mainForm->IsDisposed)
			{
				...
				mainForm->Show(Command::_hWndRevit);
			}
		}
		catch (Exception^ ex)
		{
